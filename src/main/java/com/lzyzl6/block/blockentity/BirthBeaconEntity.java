    ...
    public static void tick(Level level, BlockPos blockPos, BlockState blockState, BirthBeaconEntity birthBeaconEntity) {
        ...
        if(blockState.getValue(CHARGED)) {
            ...
            if (uuid != null) {
                ...
                level.getEntitiesOfClass(WanderingSpirit.class,AABB.ofSize(Vec3.ZERO, level.getWorldBorder().getAbsoluteMaxSize() * 2,  8388608, level.getWorldBorder().getAbsoluteMaxSize() * 2))
                .forEach(spirit -> {
